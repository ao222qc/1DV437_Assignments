...
namespace FireAndExplosions
{
    class Smoke
    {
        ...
        public void ReuseParticle()
        {
            ...
            fade = 1f;
            size = minSize;
            randomDirection = new Vector2((float)rand.NextDouble() - 0.5f, (float)rand.NextDouble() - 0.3f);//(float)rand.NextDouble() - 0.5f);
            randomDirection.Normalize();
            randomDirection = randomDirection * ((float)rand.NextDouble() * 0.12f);
